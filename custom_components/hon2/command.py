...
class Hon2Command:
    ...
    def _create_parameters(self, parameters):
        ...
        for parameter, attributes in parameters.items():
            match attributes.get("typology"):
                case "range":
                    result[parameter] = Hon2ParameterRange(parameter, attributes)
                case "enum":
                    result[parameter] = Hon2ParameterEnum(parameter, attributes)
                case "fixed":
                    result[parameter] = Hon2ParameterFixed(parameter, attributes)
        if self._multi:
            result["program"] = Hon2ParameterProgram("program", self)
        return result
    ...
    def _get_settings_keys(self, command=None):
        ...
        for key, parameter in command._parameters.items():
            if isinstance(parameter, Hon2ParameterFixed):
                continue
            if key not in keys:
                keys.append(key)
        return keys
    ...
    def dump(self):
        ...
        for key, parameter in self._parameters.items():
            if isinstance(parameter, Hon2ParameterFixed) or key == "program":
                continue
