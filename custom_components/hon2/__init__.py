...
from .hon2 import Hon2Connection, get_hOn_mac
...
CONFIG_SCHEMA = vol.Schema(
    {DOMAIN: vol.Schema(vol.All(cv.ensure_list, [HON2_SCHEMA]))},
...
async def async_setup_entry(hass: HomeAssistantType, entry: ConfigEntry):
    hon2 = Hon2Connection(hass, entry)
    await hon2.async_authorize()

    # Log all appliances
    _LOGGER.debug(f"Appliances: {hon2.appliances}")

    hass.data.setdefault(DOMAIN, {})
    hass.data[DOMAIN][entry.unique_id] = hon2

    for appliance in hon2.appliances:
        
        coordinator = await hon2.async_get_coordinator(appliance)
        coordinator.device = Hon2Device(hon2, coordinator, appliance)
    ...
    async def handle_oven_start(call):
        ...
        return await hon2.async_set(mac, "OV", parameters)
 ...
 #           "prStrDisp": call.data.get("string_display"),
        ...
        return await hon2.async_set(mac, "DW", parameters)
    ...
    async def handle_washingmachine_start(call):
        ...
        json = await hon2.async_get_state(mac, "WM")

        if json["category"] != "DISCONNECTED":
            return await hon2.async_set(mac, "WM", parameters)
        _LOGGER.error(f"This hOn2 device is disconnected - Mac address [{mac}]")
    ...
    async def handle_purifier_start(call):
        ...
        return await hon2.async_set(mac, "AP", parameters)
    ...
    # Generic method to set a mode to any hOn2 device
    async def handle_set_mode(call):
        #parameters = {"onOffStatus": "1", "machMode": call.data.get("mode", 1)}
        #return await hon2.async_set_parameter(call.data.get("device_id")[0], parameters)
        device_id = call.data.get("device")
        mac = get_hOn_mac(device_id, hass)
        coordinator = await hon2.async_get_existing_coordinator(mac)
        parameters = {"onOffStatus": "1", "machMode": call.data.get("mode", 1)}
        await coordinator.async_set(parameters)
        await coordinator.async_request_refresh()

    # Generic method to TURN OFF any hOn2 device
    ...
    async def handle_light_on(call):
        ...
        #parameters  = {"lightStatus": "1"}
        #await hon2.async_set(mac, type_name, parameters)
