...
from .command import Hon2Command
from .parameter import Hon2ParameterFixed, Hon2ParameterEnum

_LOGGER = logging.getLogger(__name__)

class Hon2Device(CoordinatorEntity):
    def __init__(self, hon2, coordinator, appliance) -> None:
    ...
    async def load_context(self):
        data = await self._hon2.async_get_context(self)
    ...
    def update_command(self, command, parameters):
        for key in command.parameters.keys():
            if( key in parameters 
                ...
                and not isinstance(command.parameters.get(key), Hon2ParameterFixed)):

                if( isinstance(command.parameters.get(key), Hon2ParameterEnum) and parameters.get(key) not in command.parameters.get(key).values): 
    ...
    async def load_commands(self):
        commands = await self._hon2.load_commands(self._appliance)
        ...
        for command, attr in commands.items():
            if "parameters" in attr:
                self._commands[command] = Hon2Command(command, attr, self._hon2, self)
            if "setParameters" in attr and "parameters" in attr[list(attr)[0]]:
                self._commands[command] = Hon2Command(command, attr.get("setParameters"), self._hon2, self)
            elif "parameters" in attr[list(attr)[0]]:
                multi = {}
                for program, attr2 in attr.items():
                    program = program.split(".")[-1].lower()
                    cmd = Hon2Command(command, attr2, self._hon2, self, multi=multi, program=program)
                    multi[program] = cmd
                    self._commands[command] = cmd

    async def load_statistics(self):
        self._statistics = await self._hon2.load_statistics(self)
