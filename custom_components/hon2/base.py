...
class Hon2BaseCoordinator(DataUpdateCoordinator):
    def __init__(self, hass, hon2, appliance):
    ...
    async def _async_update_data(self):
        #data = await self._hon2.async_get_context(self._device)
        await self._device.load_context()

        #data = await self._hon2.async_get_state(self._mac, self._type_name)
    ...
    async def async_set(self, parameters):
        await self._hon2.async_set(self._mac, self._type_name, parameters)
...
class Hon2BaseBinarySensorEntity(CoordinatorEntity, BinarySensorEntity):
...
class Hon2BaseSensorEntity(CoordinatorEntity, SensorEntity):
    def __init__(self, coordinator, appliance, key, sensor_name) -> None:
        super().__init__(coordinator)
        self._coordinator   = coordinator
        self._mac           = appliance["macAddress"]
