...
from .base import Hon2BaseCoordinator, Hon2BaseSensorEntity
...
async def async_setup_entry(hass, entry: ConfigEntry, async_add_entities) -> None:
    ...
    for appliance in hon2.appliances:
        ...
        if device.has("machMode"):
            appliances.extend([Hon2BaseMode(hass, coordinator, entry, appliance)])
        
        if device.has("temp"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "temp",        "Temperature")])
        if device.has("tempEnv"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "tempEnv",     "Environment temperature")])
        if device.has("tempIndoor"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "tempIndoor",  "Indoor temperature")])
        if device.has("tempOutdoor"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "tempOutdoor", "Outdoor temperature")])
        if device.has("tempSel"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "tempSel",     "Selected temperature")])
        if device.has("tempSelZ1"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "tempSelZ1",   "Selected temperature zone 1")])
        if device.has("tempSelZ2"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "tempSelZ2",   "Selected temperature zone 2")])
        if device.has("tempZ1"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "tempZ1",      "Temperature zone 1")])
        if device.has("tempZ2"):
            appliances.extend([Hon2BaseTemperature(hass, coordinator, entry, appliance, "tempZ2",      "Temperature zone 2")])

        if device.has("remainingTimeMM"):
            appliances.extend([Hon2BaseStart(hass, coordinator, entry, appliance)])
            appliances.extend([Hon2BaseEnd(hass, coordinator, entry, appliance)])
            appliances.extend([Hon2BaseRemainingTime(hass, coordinator, entry, appliance)])

        if device.has("humidity") and device.getInt("humidity") > 0:
            appliances.extend([Hon2BaseHumidity(hass, coordinator, entry, appliance, "", "")])
        if device.has("humidityZ1") and device.getInt("humidityZ1") > 0:
            appliances.extend([Hon2BaseHumidity(hass, coordinator, entry, appliance, "Z1", "zone 1")])
        if device.has("humidityZ2") and device.getInt("humidityZ2") > 0:
            appliances.extend([Hon2BaseHumidity(hass, coordinator, entry, appliance, "Z2", "zone 2")])
        if device.has("humidityIndoor") and device.getInt("humidityIndoor") > 0:
            appliances.extend([Hon2BaseHumidity(hass, coordinator, entry, appliance, "Indoor", "indoor")])
        if device.has("humidityOutdoor") and device.getInt("humidityOutdoor") > 0:
            appliances.extend([Hon2BaseHumidity(hass, coordinator, entry, appliance, "Outdoor", "outdoor")])
        if device.has("humidityEnv") and device.getInt("humidityEnv") > 0:
            appliances.extend([Hon2BaseHumidity(hass, coordinator, entry, appliance, "Env", "environment")])

        if device.has("pm2p5ValueIndoor") and device.getFloat("pm2p5ValueIndoor") > 0:
            appliances.extend([Hon2BaseIndoorPM2p5(hass, coordinator, entry, appliance)])
        if device.has("pm10ValueIndoor") and device.getFloat("pm10ValueIndoor") > 0:
            appliances.extend([Hon2BaseIndoorPM10(hass, coordinator, entry, appliance)])

        if device.has("vocValueIndoor") and device.getFloat("vocValueIndoor") > 0:
            appliances.extend([Hon2BaseIndoorVOC(hass, coordinator, entry, appliance)])

        if device.has("coLevel"):
            appliances.extend([Hon2BaseCOlevel(hass, coordinator, entry, appliance)])
        if device.has("airQuality") and device.getFloat("airQuality") > 0:
            appliances.extend([Hon2BaseAIRquality(hass, coordinator, entry, appliance)])
        if device.has("mainFilterStatus"):
            appliances.extend([Hon2BaseMainFilter(hass, coordinator, entry, appliance)])
        if device.has("preFilterStatus"):
            appliances.extend([Hon2BasePreFilter(hass, coordinator, entry, appliance)])

        if device.has("dryLevel"):
            appliances.extend([Hon2BaseDryLevel(hass, coordinator, entry, appliance)])
        if device.has("prCode"):
            appliances.extend([Hon2BaseProgram(hass, coordinator, entry, appliance)])
        if device.has("prPhase"):
            appliances.extend([Hon2BaseProgramPhase(hass, coordinator, entry, appliance)])
        if device.has("prTime"):
            appliances.extend([Hon2BaseProgramDuration(hass, coordinator, entry, appliance)])

        if device.has("totalWaterUsed") and device.has("totalWashCycle"):
            appliances.extend([Hon2BaseMeanWaterConsumption(hass, coordinator, entry, appliance)])
        if device.has("totalElectricityUsed") and device.getFloat("totalElectricityUsed") > 0:
            appliances.extend([Hon2BaseTotalElectricityUsed(hass, coordinator, entry, appliance)])
        if device.has("totalWashCycle"):
            appliances.extend([Hon2BaseTotalWashCycle(hass, coordinator, entry, appliance)])
        if device.has("totalWaterUsed"):
            appliances.extend([Hon2BaseTotalWaterUsed(hass, coordinator, entry, appliance)])
        if device.has("actualWeight"):
            appliances.extend([Hon2BaseWeight(hass, coordinator, entry, appliance)])


        if device.has("currentWaterUsed"):
            appliances.extend([Hon2BaseCurrentWaterUsed(hass, coordinator, entry, appliance)])
        if device.has("errors"):
            appliances.extend([Hon2BaseError(hass, coordinator, entry, appliance)])
        if device.has("currentElectricityUsed"):
            appliances.extend([Hon2BaseCurrentElectricityUsed(hass, coordinator, entry, appliance)])
        if device.has("spinSpeed"):
            appliances.extend([Hon2BaseSpinSpeed(hass, coordinator, entry, appliance)])


        # Fridge other values
        if device.has("quickModeZ1"):
            appliances.extend([Hon2BaseInt(hass, coordinator, entry, appliance, "quickModeZ1", "Quick mode Zone 1", )])
        if device.has("quickModeZ2"):
            appliances.extend([Hon2BaseInt(hass, coordinator, entry, appliance, "quickModeZ2", "Quick mode Zone 2", )])
        if device.has("intelligenceMode"):
            appliances.extend([Hon2BaseInt(hass, coordinator, entry, appliance, "intelligenceMode", "Intelligence mode", )])
        if device.has("holidayMode"):
            appliances.extend([Hon2BaseInt(hass, coordinator, entry, appliance, "holidayMode", "Holiday mode", )])
        if device.has("sterilizationStatus"):
            appliances.extend([Hon2BaseInt(hass, coordinator, entry, appliance, "sterilizationStatus", "Sterilization status", )])

        programName = device.getProgramName()
        if( programName != None ):
            appliances.extend([Hon2BaseProgramName(hass, coordinator, entry, appliance)])
...
class Hon2BaseMode(Hon2BaseSensorEntity):
...
class Hon2BaseProgramName(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseTemperature(Hon2BaseSensorEntity):
...
class Hon2BaseHumidity(Hon2BaseSensorEntity):
...
class Hon2BaseInt(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance, key, name) -> None:
        super().__init__(coordinator, appliance, key, name)
        #self._attr_icon         = "mdi:washing-machine"


class Hon2BaseRemainingTime(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseIndoorPM2p5(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseIndoorPM10(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseIndoorVOC(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseCOlevel(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseAIRquality(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BasePreFilter(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseMainFilter(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseProgram(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseProgramPhase(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseProgramDuration(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseDryLevel(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseStart(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseEnd(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseMeanWaterConsumption(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseTotalElectricityUsed(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseTotalWashCycle(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseTotalWaterUsed(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseWeight(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseCurrentWaterUsed(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseError(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseCurrentElectricityUsed(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
...
class Hon2BaseSpinSpeed(Hon2BaseSensorEntity):
    def __init__(self, hass, coordinator, entry, appliance) -> None:
