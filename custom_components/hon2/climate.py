...
async def async_setup_entry(hass, entry: ConfigEntry, async_add_entities) -> None:
    ...
    for appliance in hon2.appliances:
        if appliance['applianceTypeId'] == 11:
            ...
            appliances.append(Hon2ClimateEntity(hass, coordinator, entry, appliance))
...
class Hon2ClimateEntity(CoordinatorEntity, ClimateEntity):
    def __init__(self,hass, coordinator, entry, appliance) -> None:
        super().__init__(coordinator)
        self._coordinator   = coordinator
        self._hon2           = hass.data[DOMAIN][entry.unique_id]
        self._hass          = hass
        ...
        ''' hon2 specific values '''
        parameters = self._device.settings_command().parameters
        self._hon2_fan_modes = parameters.get('windSpeed').values
        for fan_mode in self._hon2_fan_modes:
