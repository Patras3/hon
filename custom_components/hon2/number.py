...
async def async_setup_entry(hass, entry, async_add_entities) -> None:
    ...
    for appliance in hon2.appliances:
        ...
        for key in coordinator.device.settings:
            ...
            if(isinstance(parameter, Hon2ParameterRange)
            and key.startswith("startProgram.")):
                ...
                description = NumberEntityDescription(
                    key=key,
                    name=translations.get(f"component.hon2.entity.number.{translation_key}.name", parameter.key),
                    entity_category=EntityCategory.CONFIG,
                    translation_key = translation_key,
                    icon=default_value.get("icon", None),
                    unit_of_measurement=default_value.get("unit_of_measurement", None),
                )
                appliances.extend([Hon2Number(hon2, coordinator, appliance, description)])


    async_add_entities(appliances)


class Hon2Number(Hon2Device, NumberEntity):
    def __init__(self, hon2, coordinator, appliance, description) -> None:
        ...
        if isinstance(self._data, Hon2ParameterRange):
    ...
    def _handle_coordinator_update(self):
        ...
        if isinstance(setting, Hon2ParameterRange):
