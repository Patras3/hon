...
from .base import Hon2BaseCoordinator



class Hon2Connection:
    ...
    async def async_get_coordinator(self, appliance):
        ...
        if mac in self._coordinator_dict:
            return self._coordinator_dict[mac]
        coordinator = Hon2BaseCoordinator( self._hass, self, appliance)
    ...
    async def async_authorize(self):
        ...
        async with self._session.post(f"{API_URL}/auth/v1/login", headers=post_headers, json=data) as resp:
            try:
                ...
                text = await resp.text()
                _LOGGER.error("hOn2 Invalid Data ["+ str(resp.text()) + "] after sending command ["+ str(data)+ "] with headers [" + str(post_headers) + "]. Response: " + text)
                return False
        ...
        async with self._session.get(url,headers=self._headers) as resp:
            try:
                ...
                _LOGGER.error("hOn2 Invalid Data ["+ str(resp.text()) + "] after GET [" + url + "]")
                return False
    ...
    async def async_get_context(self, device):
        ...
        # Create a new hOn2 session to avoid reaching the expiration
    ...
    async def async_set(self, mac, typeName, parameters):
        ...
        async with self._session.post(f"{API_URL}/commands/v1/send",headers=self._headers,json=command,) as resp:
            try:
                ...
                _LOGGER.error("hOn2 Invalid Data ["+ str(resp.text()) + "] after sending command ["+ str(command)+ "]")
                return False
            if data["payload"]["resultCode"] == "0":
                return True
            _LOGGER.error("hOn2 command has been rejected. Error message ["+ str(data) + "] sent command ["+ str(command)+ "]")
        return False
    ...
    async def send_command(self, device, command, parameters, ancillary_parameters):
        ...
        async with self._session.post(url, headers=self._headers, json=command) as resp:
            ...
            if data["payload"]["resultCode"] == "0":
                return True
            _LOGGER.error("hOn2 command has been rejected. Error message ["+ str(data) + "] sent data ["+ str(command)+ "]")
        return False
